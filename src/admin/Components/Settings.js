...
const Settings = ({ deleteDataOnUninstall, uninstallNonce, adminUrl }) => {
	...
	const handleToggle = () => {
		...
		if (newValue) {
			const confirmed = window.confirm(
				__('Are you sure? This will permanently delete all Video Gallery Block posts and settings when the plugin is uninstalled.', 'video-gallery-block')
			);
		...
		fetch(`${adminUrl}admin-ajax.php`, {
			...
			.then((res) => {
				if (res.success) {
					...
					setNotice(res.data?.message || __('Failed to save setting.', 'video-gallery-block'));
				}
			})
			.catch(() => {
				setNotice(__('Failed to save setting. Network error.', 'video-gallery-block'));
			})
			.finally(() => {
				setIsLoading(false);
			});
	...
	return <div className='bPlDashboardSettings bPlDashboardCard'>
		...
		<p>{__('When enabled, all plugin data will be permanently deleted when you uninstall (delete) the plugin. This includes:', 'video-gallery-block')}</p>

		<ul>
			<li>{__('All video gallery block posts', 'video-gallery-block')}</li>
			<li>{__('All plugin settings and configurations', 'video-gallery-block')}</li>
		</ul>

		<p className='settingsWarning'>
			{__('⚠️ This action cannot be undone. Your data will be safe if you only deactivate the plugin.', 'video-gallery-block')}
		</p>
